...
public class StudentService : IStudentService
{
    ...
    public async Task<StudentDto> CreateAsync(CreateStudentDto dto)
    {
        ...
        var masterStudent = await _studentMasterListRepository.GetByIdAsync(
            dto.MasterStudentId);

        if (masterStudent is null)
            throw new InvalidOperationException("Master student does not exist.");

        if (!masterStudent.IsActive)
            throw new InvalidOperationException("Master student is inactive.");

        var exists = await _studentRepository.ExistsByMasterStudentIdAsync(
            dto.MasterStudentId);
    ...
    public async Task<StudentDto?> UpdateHostelEligibilityAsync(
        int userId,
        string gender)
    {
        if (userId <= 0)
        ...
        var normalizedGender = NormalizeHostelEligibility(gender);
    ...
    private static string NormalizeHostelEligibility(string gender)
    {
        if (string.IsNullOrWhiteSpace(gender))
            throw new ArgumentException("Hostel eligibility is required.");
        ...
        throw new ArgumentException(
            "Hostel eligibility must be Male or Female.");
    }

    private static void ValidateCreateRequest(CreateStudentDto dto)
    {
    ...
    private static void ValidateUpdateRequest(UpdateStudentDto dto)
    {
        if (string.IsNullOrWhiteSpace(dto.FirstName))
            throw new ArgumentException("First name is required.");
